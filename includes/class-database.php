...
class Nexus_AI_WP_Translator_Database {
    ...
    public static function create_tables() {
        ...
        // Manual links table for linking posts across languages
        $sql_manual_links = "CREATE TABLE {$wpdb->prefix}nexus_ai_wp_manual_links (
            id bigint(20) NOT NULL AUTO_INCREMENT,
            post_id bigint(20) NOT NULL,
            linked_post_ids text NOT NULL,
            created_at datetime DEFAULT CURRENT_TIMESTAMP,
            updated_at datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            PRIMARY KEY (id),
            KEY post_id (post_id)
    ...
    /**
     * Create manual link between posts
     */
    public function create_manual_link($post_ids) {
        if (count($post_ids) < 2) {
            return false;
        }
        
        // Remove existing links for all posts
        foreach ($post_ids as $post_id) {
            $this->wpdb->delete(
                $this->wpdb->prefix . 'nexus_ai_wp_manual_links',
                array('post_id' => $post_id),
                array('%d')
            );
        }
        
        // Create new links for each post
        foreach ($post_ids as $post_id) {
            $linked_ids = array_diff($post_ids, array($post_id)); // Exclude self
            $this->wpdb->insert(
                $this->wpdb->prefix . 'nexus_ai_wp_manual_links',
                array(
                    'post_id' => $post_id,
                    'linked_post_ids' => serialize($linked_ids)
                ),
                array('%d', '%s')
            );
        }
    ...
    /**
     * Remove manual link for a post
     */
    public function remove_manual_link($post_id) {
        // Get current links
        $current_links = $this->wpdb->get_results($this->wpdb->prepare(
            "SELECT * FROM {$this->wpdb->prefix}nexus_ai_wp_manual_links WHERE post_id = %d",
            $post_id
        ));
        ...
        $linked_post_ids = maybe_unserialize($current_links[0]->linked_post_ids);
        
        // Remove this post from all linked posts
        foreach ($linked_post_ids as $linked_id) {
            $linked_data = $this->wpdb->get_row($this->wpdb->prepare(
                "SELECT * FROM {$this->wpdb->prefix}nexus_ai_wp_manual_links WHERE post_id = %d",
                $linked_id
            ));
            
            if ($linked_data) {
                $linked_ids = maybe_unserialize($linked_data->linked_post_ids);
                $linked_ids = array_diff($linked_ids, array($post_id));
                
                if (count($linked_ids) > 0) {
                    $this->wpdb->update(
                        $this->wpdb->prefix . 'nexus_ai_wp_manual_links',
                        array('linked_post_ids' => serialize(array_values($linked_ids))),
                        array('post_id' => $linked_id),
                        array('%s'),
                        array('%d')
                    );
                } else {
        ...
        // Remove the post's own link record
        return $this->wpdb->delete(
            $this->wpdb->prefix . 'nexus_ai_wp_manual_links',
            array('post_id' => $post_id),
            array('%d')
    ...
    /**
     * Get manual links for a post
     */
    public function get_manual_links($post_id) {
        ...
        if ($result) {
            return maybe_unserialize($result->linked_post_ids);
        }
