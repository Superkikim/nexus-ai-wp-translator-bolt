...
jQuery(document).ready(function($) {
    ...
    $('#nexus-ai-wp-cancel-linking').on('click', function() {
        linkingMode = false;
        selectedPosts = [];
        $('.nexus-ai-wp-link-checkbox').prop('checked', false);
        updateLinkingUI();
        updateSelectedCount();
    });
    ...
    $('#nexus-ai-wp-save-links').on('click', function() {
        if (selectedPosts.length < 2) {
            showLinkingError('<?php _e('Please select at least 2 articles to link.', 'nexus-ai-wp-translator'); ?>');
            return;
        }
        
        // Validate languages
            // Don't show switcher on non-singular pages
            return '';
        selectedPosts.forEach(function(post) {
            if (languages.includes(post.language)) {
                duplicateLanguage = true;
                duplicateLang = post.language;
                return false;
            }
        ...
        if (duplicateLanguage) {
            showLinkingError('<?php _e('Cannot link articles with the same language. Found duplicate language: ', 'nexus-ai-wp-translator'); ?>' + duplicateLang);
            return;
        }
        
        // Save links
        var postIds = selectedPosts.map(function(post) { return post.id; });
        
        $.post(nexus_ai_wp_translator_ajax.ajax_url, {
            action: 'nexus_ai_wp_create_manual_link',
            post_ids: postIds,
            nonce: nexus_ai_wp_translator_ajax.nonce
        })
        .done(function(response) {
            if (response.success) {
                alert('<?php _e('Articles linked successfully!', 'nexus-ai-wp-translator'); ?>');
                location.reload();
            } else {
                showLinkingError(response.data || '<?php _e('Failed to link articles.', 'nexus-ai-wp-translator'); ?>');
            }
        })
        .fail(function() {
            showLinkingError('<?php _e('Network error occurred.', 'nexus-ai-wp-translator'); ?>');
        });
    });
    ...
    $(document).on('change', '.nexus-ai-wp-link-checkbox', function() {
        ...
        var postLanguage = $(this).data('language');
        
        if ($(this).is(':checked')) {
            selectedPosts.push({
        // Only show language switcher if on singular page with manual links
                title: postTitle,
                language: postLanguage
            $manual_links = $this->get_manually_linked_posts($post->ID);
            
            if (empty($manual_links)) {
                // No manual links, don't show switcher
                return '';
            }
            
            $available_languages = $this->get_post_available_languages($post->ID);
        } else {
            selectedPosts = selectedPosts.filter(function(post) {
                return post.id !== postId;
        // Get current post language for proper selection
        global $post;
        $current_post_lang = get_post_meta($post->ID, '_nexus_ai_wp_translator_language', true) ?: get_option('nexus_ai_wp_translator_source_language', 'en');
        ...
        updateSelectedCount();
        hideLinkingError();
                $selected = ($lang === $current_post_lang) ? 'selected' : '';
    
    // Unlink post
    $(document).on('click', '.unlink-post-btn', function() {
        var button = $(this);
        var postId = button.data('post-id');
        var postTitle = button.data('post-title');
                // Get direct URL to translated post
        if (!confirm('<?php _e('Are you sure you want to unlink', 'nexus-ai-wp-translator'); ?> "' + postTitle + '"?\n\n<?php _e('This will remove all manual links for this article.', 'nexus-ai-wp-translator'); ?>')) {
            return;
        }
        ...
        $.post(nexus_ai_wp_translator_ajax.ajax_url, {
            action: 'nexus_ai_wp_remove_manual_link',
            post_id: postId,
            nonce: nexus_ai_wp_translator_ajax.nonce
        })
        .done(function(response) {
                $class = ($lang === $current_post_lang) ? 'current' : '';
                alert('<?php _e('Article unlinked successfully!', 'nexus-ai-wp-translator'); ?>');
                location.reload();
            } else {
                alert('<?php _e('Failed to unlink article:', 'nexus-ai-wp-translator'); ?> ' + (response.data || '<?php _e('Unknown error', 'nexus-ai-wp-translator'); ?>'));
            }
        })
                // Get direct URL to translated post
