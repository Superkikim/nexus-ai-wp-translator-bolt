...
class Nexus_AI_WP_Translator_Admin {
    ...
    private function init_hooks() {
        ...
        add_action('wp_ajax_nexus_ai_wp_save_manual_links', array($this, 'ajax_save_manual_links'));
        add_action('wp_ajax_nexus_ai_wp_remove_manual_links', array($this, 'ajax_remove_manual_links'));
        ...
        add_action('wp_ajax_nexus_ai_wp_create_manual_link', array($this, 'ajax_create_manual_link'));
        add_action('wp_ajax_nexus_ai_wp_remove_manual_link', array($this, 'ajax_remove_manual_link'));
    ...
    public function render_posts_list($post_type) {
        ...
        $output .= '<thead>';
        $output .= '<tr>';
        $output .= '<th class="check-column"><input type="checkbox" id="nexus-ai-wp-select-all" style="display: none;"></th>';
        $output .= '<th>' . __('Title', 'nexus-ai-wp-translator') . '</th>';
        $output .= '<th>' . __('Language', 'nexus-ai-wp-translator') . '</th>';
        $output .= '<th>' . __('Linked Articles', 'nexus-ai-wp-translator') . '</th>';
        ...
        foreach ($posts as $post) {
            ...
            // Get manually linked posts
            $linked_posts = $this->db->get_manual_links($post->ID);
            $linked_count = count($linked_posts);
            
            $output .= '<tr>';
            $output .= '<td class="check-column">';
            $output .= '<input type="checkbox" class="nexus-ai-wp-link-checkbox" value="' . $post->ID . '" data-language="' . esc_attr($post_language) . '" data-title="' . esc_attr($post->post_title) . '" style="display: none;">';
            $output .= '</td>';
            $output .= '<td>';
            ...
            $output .= '<td>';
            if ($linked_count > 0) {
                $output .= sprintf(__('%d linked articles', 'nexus-ai-wp-translator'), $linked_count);
                $output .= '<br><small>';
                $linked_languages = array();
                foreach ($linked_posts as $linked_id) {
                    $linked_lang = get_post_meta($linked_id, '_nexus_ai_wp_translator_language', true) ?: get_option('nexus_ai_wp_translator_source_language', 'en');
                    $linked_languages[] = $linked_lang;
                }
                $output .= __('Languages: ', 'nexus-ai-wp-translator') . implode(', ', array_unique($linked_languages));
                $output .= '</small>';
            } else {
            ...
            $output .= '<td>';
            if ($linked_count > 0) {
                $output .= '<button type="button" class="button button-secondary unlink-post-btn" ';
                $output .= 'data-post-id="' . $post->ID . '" ';
                $output .= 'data-post-title="' . esc_attr($post->post_title) . '">';
                $output .= __('Unlink', 'nexus-ai-wp-translator');
                $output .= '</button> ';
            }
    ...
    /**
     * AJAX: Save manual links between posts
     */
    public function ajax_save_manual_links() {
        check_ajax_referer('nexus_ai_wp_translator_nonce', 'nonce');
        ...
        if (count($post_ids) < 2) {
            wp_send_json_error(__('At least 2 posts are required to create a link group', 'nexus-ai-wp-translator'));
        }
        
        // Validate that all posts exist and have different languages
        $languages = array();
        $source_language = get_option('nexus_ai_wp_translator_source_language', 'en');
        ...
        foreach ($post_ids as $post_id) {
            ...
            if (!$post) {
                wp_send_json_error(sprintf(__('Post with ID %d not found', 'nexus-ai-wp-translator'), $post_id));
            }
            
            $post_lang = get_post_meta($post_id, '_nexus_ai_wp_translator_language', true) ?: $source_language;
            
            if (in_array($post_lang, $languages)) {
                wp_send_json_error(sprintf(__('Multiple posts have the same language (%s). Each linked group must contain posts in different languages only.', 'nexus-ai-wp-translator'), $post_lang));
            }
            
            $languages[] = $post_lang;
        }
        
        // Create the manual links
        $db = Nexus_AI_WP_Translator_Database::get_instance();
        $result = $db->create_manual_link($post_ids);
        ...
        if ($result) {
            ...
            $db->log_translation_activity(
                $post_ids[0], 
                'manual_link_created', 
                'completed', 
                sprintf(__('Manual links created between: %s', 'nexus-ai-wp-translator'), implode(', ', $post_titles))
            );
            
            wp_send_json_success(__('Links created successfully', 'nexus-ai-wp-translator'));
        } else {
            wp_send_json_error(__('Failed to create links', 'nexus-ai-wp-translator'));
        }
    }
    
    /**
     * AJAX: Remove manual links for a post
     */
    public function ajax_remove_manual_links() {
        check_ajax_referer('nexus_ai_wp_translator_nonce', 'nonce');
        ...
        $db = Nexus_AI_WP_Translator_Database::get_instance();
        $result = $db->remove_manual_link($post_id);
        ...
        if ($result) {
            ...
            $db->log_translation_activity(
                ...
                'manual_link_removed', 
                'completed', 
                sprintf(__('Manual links removed for: %s', 'nexus-ai-wp-translator'), $post ? $post->post_title : "Post ID {$post_id}")
            );
            
            wp_send_json_success(__('Links removed successfully', 'nexus-ai-wp-translator'));
        } else {
            wp_send_json_error(__('Failed to remove links or no links found', 'nexus-ai-wp-translator'));
        }
    }
    ...
    /**
     * AJAX: Create manual link between posts
     */
    public function ajax_create_manual_link() {
        check_ajax_referer('nexus_ai_wp_translator_nonce', 'nonce');
        ...
        if (count($post_ids) < 2) {
            wp_send_json_error(__('At least 2 articles must be selected for linking.', 'nexus-ai-wp-translator'));
            return;
        }
        ...
        foreach ($post_ids as $post_id) {
            ...
            if (!$post || $post->post_status !== 'publish') {
                wp_send_json_error(sprintf(__('Post ID %d is not valid or not published.', 'nexus-ai-wp-translator'), $post_id));
                return;
            }
            
            $post_lang = get_post_meta($post_id, '_nexus_ai_wp_translator_language', true) ?: get_option('nexus_ai_wp_translator_source_language', 'en');
            
            if (in_array($post_lang, $languages)) {
                wp_send_json_error(sprintf(__('Multiple articles with the same language (%s) cannot be linked together. Each linked group must contain articles in different languages.', 'nexus-ai-wp-translator'), $post_lang));
                return;
            }
        ...
        $result = $this->db->create_manual_link($post_ids);
        
        if ($result) {
            $this->db->log_translation_activity($post_ids[0], 'manual_link_created', 'completed', 
                sprintf('Manually linked %d articles: %s', count($post_ids), implode(', ', $languages)));
            
            wp_send_json_success(array(
                'message' => sprintf(__('Successfully linked %d articles across different languages.', 'nexus-ai-wp-translator'), count($post_ids)),
                'linked_posts' => $valid_posts
            ));
        } else {
            wp_send_json_error(__('Failed to create manual link.', 'nexus-ai-wp-translator'));
        }
    }
    
    /**
     * AJAX: Remove manual link for a post
     */
    public function ajax_remove_manual_link() {
        check_ajax_referer('nexus_ai_wp_translator_nonce', 'nonce');
        ...
        $result = $this->db->remove_manual_link($post_id);
        
        if ($result) {
            $this->db->log_translation_activity($post_id, 'manual_link_removed', 'completed', 'Manual link removed');
            wp_send_json_success(__('Manual link removed successfully.', 'nexus-ai-wp-translator'));
        } else {
            wp_send_json_error(__('Failed to remove manual link or no link exists.', 'nexus-ai-wp-translator'));
        }
    }
