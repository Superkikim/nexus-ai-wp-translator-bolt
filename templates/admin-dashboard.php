...
<div class="wrap">
    ...
    <div class="nexus-ai-wp-content-tabs">
        ...
        <div id="articles-tab" class="tab-content active">
            ...
            <div class="nexus-ai-wp-manual-linking">
                <h3><?php _e('Manual Article Linking', 'nexus-ai-wp-translator'); ?></h3>
                <p class="description">
                    <?php _e('Select articles to link together across different languages. Each linked group must contain articles in different languages (minimum 2 articles).', 'nexus-ai-wp-translator'); ?>
                </p>
                
                <div class="linking-controls">
                    <button type="button" id="nexus-ai-wp-start-linking" class="button button-primary">
                        <?php _e('Start Linking Articles', 'nexus-ai-wp-translator'); ?>
                    </button>
                    <button type="button" id="nexus-ai-wp-cancel-linking" class="button" style="display: none;">
                        <?php _e('Cancel', 'nexus-ai-wp-translator'); ?>
                    </button>
                    <button type="button" id="nexus-ai-wp-save-links" class="button button-primary" style="display: none;">
                        <?php _e('Save Links', 'nexus-ai-wp-translator'); ?>
                    </button>
                </div>
                
                <div id="nexus-ai-wp-linking-status" class="linking-status" style="display: none;">
                    <p><strong><?php _e('Linking Mode Active', 'nexus-ai-wp-translator'); ?></strong></p>
                    <p class="description"><?php _e('Select articles to link together. Selected articles: ', 'nexus-ai-wp-translator'); ?><span id="selected-count">0</span></p>
                    <div id="selected-articles"></div>
                    <div id="linking-errors" class="notice notice-error" style="display: none;"></div>
                </div>
            </div>
...
jQuery(document).ready(function($) {
    ...
    // Remove manual links for a post
    $(document).on('click', '.remove-links-btn', function() {
        var button = $(this);
        var postId = button.data('post-id');
        var postTitle = button.data('post-title');

        if (!confirm('<?php _e('Are you sure you want to remove all manual links for', 'nexus-ai-wp-translator'); ?> "' + postTitle + '"?\n\n<?php _e('This will unlink this post from all other posts. This action cannot be undone.', 'nexus-ai-wp-translator'); ?>')) {
            return;
        }

        button.prop('disabled', true).text('<?php _e('Removing...', 'nexus-ai-wp-translator'); ?>');

        $.post(nexus_ai_wp_translator_ajax.ajax_url, {
            action: 'nexus_ai_wp_remove_manual_links',
            post_id: postId,
            nonce: nexus_ai_wp_translator_ajax.nonce
        })
        .done(function(response) {
            if (response.success) {
                alert('<?php _e('Links removed successfully!', 'nexus-ai-wp-translator'); ?>');
                location.reload();
            } else {
                alert('<?php _e('Failed to remove links:', 'nexus-ai-wp-translator'); ?> ' + (response.data || response.message || '<?php _e('Unknown error', 'nexus-ai-wp-translator'); ?>'));
            }
        })
        .fail(function() {
            alert('<?php _e('Network error occurred', 'nexus-ai-wp-translator'); ?>');
        ...
        .always(function() {
            button.prop('disabled', false).text('<?php _e('Remove Links', 'nexus-ai-wp-translator'); ?>');
        });
    });
    ...
    var NexusAIWPTranslatorDashboard = {
        ...
        initManualLinking: function() {
            var self = this;
            
            // Start linking mode
            $('#nexus-ai-wp-start-linking').on('click', function() {
                self.startLinkingMode();
            });
            
            // Cancel linking mode
            $('#nexus-ai-wp-cancel-linking').on('click', function() {
                self.cancelLinkingMode();
            });
            
            // Save links
            $('#nexus-ai-wp-save-links').on('click', function() {
                self.saveLinks();
            });
            
            // Handle article selection
            $(document).on('change', '.nexus-ai-wp-link-checkbox', function() {
                self.handleArticleSelection($(this));
            });
        },
        
        startLinkingMode: function() {
            console.log('Starting linking mode');
            this.linkingMode = true;
            this.selectedArticles = [];
            
            // Update UI
            $('#nexus-ai-wp-start-linking').hide();
            $('#nexus-ai-wp-cancel-linking, #nexus-ai-wp-save-links').show();
            $('#nexus-ai-wp-linking-status').show();
            
            // Add checkboxes to article rows
            this.addLinkingCheckboxes();
            
            this.updateSelectedCount();
        },
        
        cancelLinkingMode: function() {
            console.log('Canceling linking mode');
            this.linkingMode = false;
            this.selectedArticles = [];
            
            // Update UI
            $('#nexus-ai-wp-start-linking').show();
            $('#nexus-ai-wp-cancel-linking, #nexus-ai-wp-save-links').hide();
            $('#nexus-ai-wp-linking-status').hide();
            
            // Remove checkboxes
            $('.nexus-ai-wp-link-checkbox').remove();
            $('.nexus-ai-wp-link-cell').remove();
            
            this.clearErrors();
        },
        
        addLinkingCheckboxes: function() {
            var self = this;
            
            // Add header
            $('.wp-list-table thead tr').prepend('<th class="nexus-ai-wp-link-cell"><?php _e('Link', 'nexus-ai-wp-translator'); ?></th>');
            
            // Add checkboxes to each row
            $('.wp-list-table tbody tr').each(function() {
                var row = $(this);
                var postId = self.extractPostId(row);
                var postTitle = row.find('.row-title').text().trim();
                var postLang = row.find('.post-language').text().trim() || '<?php echo get_option('nexus_ai_wp_translator_source_language', 'en'); ?>';
                
                if (postId) {
                    var checkbox = '<td class="nexus-ai-wp-link-cell">' +
                        '<input type="checkbox" class="nexus-ai-wp-link-checkbox" ' +
                        'data-post-id="' + postId + '" ' +
                        'data-post-title="' + postTitle + '" ' +
                        'data-post-lang="' + postLang + '">' +
                        '</td>';
        ...
        extractPostId: function(row) {
            // Try to extract post ID from various sources
            var editLink = row.find('a[href*="post.php?post="]').attr('href');
            if (editLink) {
                var match = editLink.match(/post=(\d+)/);
                if (match) return match[1];
            }
            
            // Try from translate button
            var translateBtn = row.find('.translate-post-btn');
            if (translateBtn.length) {
                return translateBtn.data('post-id');
            }
            
            return null;
        },
        
        handleArticleSelection: function(checkbox) {
            var postId = checkbox.data('post-id');
            var postTitle = checkbox.data('post-title');
            var postLang = checkbox.data('post-lang');
            ...
            if (checkbox.is(':checked')) {
                ...
                this.selectedArticles = this.selectedArticles.filter(function(article) {
        ...
        updateSelectedCount: function() {
            $('#selected-count').text(this.selectedArticles.length);
            
            // Update selected articles display
            var selectedHtml = '';
            this.selectedArticles.forEach(function(article) {
                selectedHtml += '<span class="selected-article-tag">' +
                    article.title + ' (' + article.language + ')' +
                    '</span>';
            });
            $('#selected-articles').html(selectedHtml);
        },
        ...
        validateSelection: function() {
            ...
            if (this.selectedArticles.length < 2) {
                return; // No validation needed for less than 2 articles
            }
            
            // Check for duplicate languages
            var languages = this.selectedArticles.map(function(article) {
            ...
            if (languages.length !== uniqueLanguages.length) {
                this.showError('<?php _e('Error: Cannot link articles with the same language. Each linked group must contain articles in different languages only.', 'nexus-ai-wp-translator'); ?>');
                return false;
            }
        ...
        showError: function(message) {
            $('#linking-errors').html('<p>' + message + '</p>').show();
        },
        
        clearErrors: function() {
            $('#linking-errors').hide().empty();
        },
        ...
        saveLinks: function() {
            ...
            // Validate selection
            if (this.selectedArticles.length < 2) {
                this.showError('<?php _e('Error: You must select at least 2 articles to create a link group.', 'nexus-ai-wp-translator'); ?>');
                return;
            }
            ...
            // Disable save button
            $('#nexus-ai-wp-save-links').prop('disabled', true).text('<?php _e('Saving...', 'nexus-ai-wp-translator'); ?>');
            
            // Prepare data
            var postIds = this.selectedArticles.map(function(article) {
                return article.id;
            });
            
            console.log('Saving links for posts:', postIds);
            
            // Send AJAX request
            $.post(nexus_ai_wp_translator_ajax.ajax_url, {
                action: 'nexus_ai_wp_save_manual_links',
                post_ids: postIds,
                nonce: nexus_ai_wp_translator_ajax.nonce
            })
            .done(function(response) {
                console.log('Save links response:', response);
                
                if (response.success) {
                    alert('<?php _e('Links saved successfully!', 'nexus-ai-wp-translator'); ?>');
                    self.cancelLinkingMode();
                    // Optionally reload the page to show updated data
                    location.reload();
                } else {
                    self.showError('<?php _e('Error saving links: ', 'nexus-ai-wp-translator'); ?>' + (response.data || response.message || '<?php _e('Unknown error', 'nexus-ai-wp-translator'); ?>'));
                }
            })
            .fail(function(xhr, status, error) {
                console.log('Save links failed:', error);
                self.showError('<?php _e('Network error occurred while saving links.', 'nexus-ai-wp-translator'); ?>');
            })
            .always(function() {
                $('#nexus-ai-wp-save-links').prop('disabled', false).text('<?php _e('Save Links', 'nexus-ai-wp-translator'); ?>');
            });
        },
    ...
    // Initialize manual linking
    NexusAIWPTranslatorDashboard.initManualLinking();
});
</script>

<style>
/* Manual Linking Styles */
.nexus-ai-wp-manual-linking {
    background: #fff;
...
.nexus-ai-wp-link-cell {
    width: 50px;
    text-align: center;
}

.nexus-ai-wp-link-checkbox {
    margin: 0;
}
